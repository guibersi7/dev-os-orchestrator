      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_320px]">
        <Card className="p-5">
          ...
          <p className="mt-3 text-sm leading-6 text-[#9AA4BA]">
            This pull request changes authentication flow behavior and has active release impact. Standup treats its reviews,
          ...
          <div className="mt-6 space-y-4">
            {workEvents.slice(0, 3).map((event) => (
              <div key={event.id} className="rounded-md border border-[#212938] p-4">
                ...
                <p className="mt-1 text-sm text-[#9AA4BA]">{event.summary}</p>
              </div>
            ))}
          </div>
        </Card>
        <Card className="p-5">
          ...
          <dl className="mt-4 space-y-3 text-sm">
            <div className="flex justify-between"><dt className="text-[#6A7489]">Reviews</dt><dd>{pr.reviews}</dd></div>
            <div className="flex justify-between"><dt className="text-[#6A7489]">Comments</dt><dd>{pr.comments}</dd></div>
            <div className="flex justify-between"><dt className="text-[#6A7489]">Changed files</dt><dd>{pr.changedFiles}</dd></div>
            <div className="flex justify-between"><dt className="text-[#6A7489]">Blocks</dt><dd>{pr.blocksRelease ?? "None"}</dd></div>
          </dl>
        </Card>
      </div>
    </div>
